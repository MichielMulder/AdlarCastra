"""Config flow for Adlar Castra Aurora 2 integration."""
...
STEP_TCP_DATA_SCHEMA = vol.Schema(
    {
        vol.Required("name", default="AdlarCastra"): str,
...
async def async_validate_device(com_type, port, slave_id, address: str | None) -> None:
    ...
    try:
        ...
        result = await client.read_holding_registers(
            HoldingRegisters.control_type, count=1, slave=int(slave_id)
        )
    ...
    if value_output not in DEVICE_TYPES:
        _LOGGER.debug(
...
class AdlarConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Adlar Castra Modbus TCP."""
    ...
    async def async_step_tcp(self, user_input: Optional[dict(str, Any)] = None):
        ...
        if user_input is not None:
            ...
            if not errors:
                ...
                self.data.update({"board_type": "AURORA"})
    ...
    async def async_step_serial(self, user_input: Optional[dict(str, Any)] = None):
        ...
        if user_input is not None:
            try:
                await async_validate_device(
                    "serial", user_input["host_port"], user_input["slave_id"], None
                )
            except ValueError as error:
                errors["base"] = str(error)
            if not errors:
